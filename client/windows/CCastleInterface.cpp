...
CBuildingRect::CBuildingRect(CCastleBuildings * Par, const CGTownInstance * Town, const CStructure * Str)
	: CShowableAnim(0, 0, Str->defName, BASE | USE_RLE),
	  parent(Par),
	  town(Town),
	  str(Str),
	  stateCounter(80)
		...
		case BuildingID::SPECIAL_1:
			...
			case ETownType::CONFLUX:
				if(town->visitingHero)
					...
					LOCPLINT->showInfoDialog(str(boost::format(CGI->generaltexth->allTexts[273]) % b->Name())); //Only visiting heroes may use the %s.
				break;
...
void CCastleBuildings::enterBlacksmith(ArtifactID artifactID)
{
	...
	if(!hero)
	{
		LOCPLINT->showInfoDialog(str(boost::format(CGI->generaltexth->allTexts[273]) % town->town->buildings.find(BuildingID::BLACKSMITH)->second->Name()));
		return;
	}
...
void CCastleBuildings::enterCastleGate()
{
	...
	GH.pushInt(new CObjectListWindow(availableTowns, gateIcon, CGI->generaltexth->jktexts[40],
	                                 CGI->generaltexth->jktexts[41], bind(&CCastleInterface::castleTeleport, LOCPLINT->castleInt, _1)));
}
...
std::string CCreaInfo::genGrowthText()
{
	...
	std::string descr = str(boost::format(CGI->generaltexth->allTexts[589]) % creature->nameSing % gi.totalGrowth());
...
{
	if(previousState && (!down))
		GH.pushInt(new CBuildWindow(town, building, state, false));
}

void CHallInterface::CBuildingBox::clickRight(tribool down, bool previousState)
{
	if(down)
		GH.pushInt(new CBuildWindow(town, building, state, true));
}
	...
	case EBuildingState::MISSING_BASE:
		{
			...
			ret = str(boost::format(msg) % town->town->buildings.at(building->upgrade)->Name());
			break;
		}
	}
	return ret;
	...
	building(Building)
{
	...
	new CLabel(197, 30, FONT_MEDIUM, CENTER, Colors::WHITE,
	           str(boost::format(CGI->generaltexth->hcommands[7]) % building->Name()));
	...
	if(!rightClick)
	{ //normal window
		std::string tooltipYes = str(boost::format(CGI->generaltexth->allTexts[595]) % building->Name());
		std::string tooltipNo = str(boost::format(CGI->generaltexth->allTexts[596]) % building->Name());
	...
	CWindowObject(PLAYER_COLORED | BORDERED, getBgName(town))
{
	...
	std::string text = str(boost::format(CGI->generaltexth->fcommands[6]) % fortBuilding->Name());
	...
	availableCount(nullptr)
{
	...
	{
		hoverText = str(boost::format(CGI->generaltexth->tcommands[21]) % getMyCreature()->namePl);
	...
	CWindowObject(PLAYER_COLORED, "TPSMITH")
{
	...
	title = new CLabel(165, 28, FONT_BIG, CENTER, Colors::YELLOW,
	                   str(boost::format(CGI->generaltexth->allTexts[274]) % creature->nameSing));
	...
	std::string text = str(boost::format(CGI->generaltexth->allTexts[595]) % creature->nameSing);
	buy = new CButton(Point(42, 312), "IBUY30.DEF", CButton::tooltip(text), [&] { close(); }, SDLK_RETURN);

	text = str(boost::format(CGI->generaltexth->allTexts[596]) % creature->nameSing);
