...
void CTerrainRect::fadeFromCurrentView()
{
	...
	if(!fadeSurface)
		fadeSurface = newSurface(pos.w, pos.h);
...
CAdvMapInt::CAdvMapInt():
	position(int3(0, 0, 0)),
	duringAITurn(false), //actual init later in changeMode
	scrollingDir(0),
	scrollingState(false),
	swipeEnabled(settings["general"]["swipe"].Bool()),
	swipeMovementRequested(false),
	swipeTargetPosition(int3(-1, -1, -1)), state(NA),
	updateScreen(false), anim(0), animValHitCount(0),
	heroAnim(0), heroAnimValHitCount(0), mode(EAdvMapMode::NORMAL), worldViewScale(0.0f), minimap(Rect(ADVOPT.minimapX, ADVOPT.minimapY, ADVOPT.minimapW, ADVOPT.minimapH)),
	statusbar(ADVOPT.statusbarX,ADVOPT.statusbarY,ADVOPT.statusbarG), heroList(ADVOPT.hlistSize, Point(ADVOPT.hlistX, ADVOPT.hlistY), ADVOPT.hlistAU, ADVOPT.hlistAD), townList(ADVOPT.tlistSize, Point(ADVOPT.tlistX, ADVOPT.tlistY), ADVOPT.tlistAU, ADVOPT.tlistAD), infoBar(Rect(ADVOPT.infoboxX, ADVOPT.infoboxY, 192, 192)),
	activeMapPanel(nullptr), spellBeingCasted(nullptr),
	selection(nullptr)
{
	adventureInt = this;
	pos.x = pos.y = 0;
	...
	for(int i = 0; i < 5; ++i)
	{
		...
		panelWorldView->addChildToPanel(new CLabel(wvLeft + 45, 263 + i * 20, FONT_SMALL, TOPLEFT,
	...
	for(int i = 0; i < 7; ++i)
	{
		...
		panelWorldView->addChildToPanel(new CLabel(wvLeft + 45, 387 + i * 20, FONT_SMALL, TOPLEFT,
		                                           Colors::WHITE, CGI->generaltexth->allTexts[619 + i]));
	}
	panelWorldView->addChildToPanel(new CLabel(wvLeft + 5, 367, FONT_SMALL, TOPLEFT,
	                                           Colors::WHITE, CGI->generaltexth->allTexts[617]));
	panelWorldView->addChildToPanel(new CLabel(wvLeft + 185, 387, FONT_SMALL, BOTTOMRIGHT,
...
const CGObjectInstance * CAdvMapInt::getActiveObject(const int3 & mapPos)
{
	...
	return *range::max_element(bobjs, &CMapHandler::compareObjectBlitOrder);
	/*
...
void CAdvMapInt::tileLClicked(const int3 & mapPos)
{
	...
	if(const CGHeroInstance * currentHero = curHero()) //hero is selected
	{
		...
		if(canSelect && pn->turns == 255) //selectable object at inaccessible tile
		{
			select(static_cast<const CArmedInstance*>(topBlocking), false);
			return;
		}
		...
		//remove old path and find a new one if we clicked on accessible tile
	...
	CWindowObject(PLAYER_COLORED, "ADVOPTS")
{
	...
	scenInfo->addCallback(showScenarioInfo);
