...
void CRecruitmentWindow::CCostBox::createItems(TResources res)
{
	...
	while(iter.valid())
	{
		...
		resources.insert(make_pair(iter->resType, std::make_pair(text, image)));
		++iter;
	}
...
void CRecruitmentWindow::select(CCreatureCard * card)
{
	...
	if(card)
	{
		...
		//Recruit %s
		title->setText(str(boost::format(CGI->generaltexth->tcommands[21]) % card->creature->namePl));
	...
	dwelling(Dwelling)
{
	...
	slider = new CSlider(Point(176, 279), 135, bind(&CRecruitmentWindow::sliderMoved, this, _1), 0, 0, 0, true);
...
void CRecruitmentWindow::availableCreaturesChanged()
{
	...
	if(!cards.empty() && selected) // find position of selected item
		selectedIndex = find(cards.begin(), cards.end(), selected) - cards.begin();
	...
	slider(nullptr)
{
	...
	leftInput = new CTextInput(Rect(20, 218, 100, 36), FONT_BIG, bind(&CSplitWindow::setAmountText, this, _1, true));
	rightInput = new CTextInput(Rect(176, 218, 100, 36), FONT_BIG, bind(&CSplitWindow::setAmountText, this, _1, false));

	//add filters to allow only number input
	leftInput->filters += bind(&CTextInput::numberFilter, _1, _2, leftMin, leftMax);
	rightInput->filters += bind(&CTextInput::numberFilter, _1, _2, rightMin, rightMax);
	...
	slider = new CSlider(Point(21, 194), 257, bind(&CSplitWindow::sliderMoved, this, _1), 0, sliderPositions, rightAmount - rightMin, true);
	...
	cb(callback)
{
	...
	new CLabel(192, 33, FONT_MEDIUM, CENTER, Colors::WHITE,
	           str(boost::format(CGI->generaltexth->allTexts[444]) % hero->name));

	//%s is now a level %d %s.
	new CLabel(192, 162, FONT_MEDIUM, CENTER, Colors::WHITE,
	           str(boost::format(CGI->generaltexth->allTexts[445]) % hero->name % hero->level % hero->type->heroClass->name));
	...
	onFullscreenChanged(settings.listen["video"]["fullscreen"])
{
	...
	heroMoveSpeed->addCallback(bind(&setIntSetting, "adventure", "heroSpeed", _1));
	...
	enemyMoveSpeed->addCallback(bind(&setIntSetting, "adventure", "enemySpeed", _1));
...
void CSystemOptionsWindow::selectGameRes()
{
	...
	GH.pushInt(new CObjectListWindow(items, nullptr, texts["label"].String(), texts["help"].String(),
	                                 bind(&CSystemOptionsWindow::setGameRes, this, _1)));
}
	...
	tavernObj(TavernObj)
{
	...
	auto rumorText = str(boost::format(CGI->generaltexth->allTexts[216]) % LOCPLINT->cb->getTavernRumor(tavernObj));
	...
	else if(LOCPLINT->castleInt && LOCPLINT->cb->howManyHeroes(true) >= VLC->modh->settings.MAX_HEROES_AVAILABLE_PER_PLAYER)
	{
		...
		recruit->addHoverText(CButton::NORMAL, str(boost::format(CGI->generaltexth->tavernInfo[1]) % LOCPLINT->cb->howManyHeroes(true)));
		recruit->block(true);
	}
	else if((!LOCPLINT->castleInt) && LOCPLINT->cb->howManyHeroes(false) >= VLC->modh->settings.MAX_HEROES_ON_MAP_PER_PLAYER)
	{
		//Cannot recruit. You already have %d Heroes.
		recruit->addHoverText(CButton::NORMAL, str(boost::format(CGI->generaltexth->tavernInfo[1]) % LOCPLINT->cb->howManyHeroes(false)));
		recruit->block(true);
	}
...
void CTavernWindow::show(SDL_Surface * to)
{
	...
	if(selected >= 0)
	{
		...
		if(selected != oldSelected && !recruit->isBlocked())
		{
			...
			//Recruit %s the %s
			recruit->addHoverText(CButton::NORMAL, str(boost::format(CGI->generaltexth->tavernInfo[3]) % sel->h->name % sel->h->type->heroClass->name));
		}

		printAtMiddleWBLoc(sel->description, 146, 395, FONT_SMALL, 200, Colors::WHITE, to);
		drawBorder(to, sel->pos.x - 2, sel->pos.y - 2, sel->pos.w + 4, sel->pos.h + 4, int3(247, 223, 123));
	}
}
...
void CExchangeWindow::prepareBackground()
{
	...
	{
		return str(boost::format(CGI->generaltexth->allTexts[138])
	...
	owner(_owner)
{
	...
	for(int g = 0; g < 12; ++g)
	{
		...
		trim_if(text, boost::algorithm::is_any_of("\""));
	...
	selected(0)
{
	...
	for(int id : _items)
	{
		items.push_back(make_pair(id, CGI->mh->map->objects[id]->getObjectName()));
	}
	...
	selected(0)
{
	...
	for(size_t i = 0; i < _items.size(); i++)
		items.push_back(make_pair(int(i), _items[i]));
...
void CObjectListWindow::init(CIntObject * titlePic, std::string _title, std::string _descr)
{
	...
	list = new CListBox(bind(&CObjectListWindow::genItem, this, _1), CListBox::DestroyFunc(),
