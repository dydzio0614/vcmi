	...
	case HERO_SECONDARY_SKILL:
		{
			...
			return 0;
			//FIXME: Should be transparent instead of empty
		}
	default:
		assert(0);
		return 0;
...
std::string InfoBoxHeroData::getHoverText()
{
	...
	{
	case HERO_PRIMARY_SKILL:
		return str(boost::format(CGI->generaltexth->heroscrn[1]) % CGI->generaltexth->primarySkillNames[index]);
	case HERO_MANA:
	...
	case HERO_SECONDARY_SKILL:
		{
			if(hero->secSkills.size() > index)
			{
				...
				return str(boost::format(CGI->generaltexth->heroscrn[21]) % level % skill);
			}
			return "";
		}
	default:
	...
	CWindowObject(PLAYER_COLORED | BORDERED, conf.go()->ac.overviewBg)
{
	...
	tabArea = new CTabbedInt(bind(&CKingdomInterface::createMainTab, this, _1), CTabbedInt::DestroyFunc(), Point(4, 4));
...
void CKingdomInterface::generateObjectsList(const std::vector<const CGObjectInstance *> & ownedObjects)
{
	...
	dwellingsList = new CListBox(bind(&CKingdomInterface::createOwnedObject, this, _1), CListBox::DestroyFunc(),
...
CKingdHeroList::CKingdHeroList(size_t maxSize)
{
	...
	heroes = new CListBox(bind(&CKingdHeroList::createHeroItem, this, _1), bind(&CKingdHeroList::destroyHeroItem, this, _1),
...
void CKingdHeroList::destroyHeroItem(CIntObject * object)
{
	...
	{
		artSets.erase(find(artSets.begin(), artSets.end(), hero->heroArts));
	}
	delete object;
}
...
CKingdTownList::CKingdTownList(size_t maxSize)
{
	...
	ui32 size = conf.go()->ac.overviewSize * 116 + 19;
	towns = new CListBox(bind(&CKingdTownList::createTownItem, this, _1), CListBox::DestroyFunc(),
	                     Point(19, 21), Point(0, 116), maxSize, townCount, 0, 1, Rect(-19, -21, size, size));
}
	...
	hero(Hero)
{
	...
	artsTabs = new CTabbedInt(bind(&CHeroItem::onTabSelected, this, _1), bind(&CHeroItem::onTabDeselected, this, _1));
	...
	artButtons->addCallback(bind(&CTabbedInt::setActive, artsTabs, _1));
	artButtons->addCallback(bind(&CHeroItem::onArtChange, this, _1));
