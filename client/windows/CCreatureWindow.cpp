...
{
	if(min != max)
		printStatString(index, name, str(boost::format("%d - %d") % min % max));
	else
		printStatString(index, name, str(boost::format("%d") % min));
}

void CStackWindow::CWindowSection::printStatBase(int index, std::string name, int base, int current)
{
	if(base != current)
		printStatString(index, name, str(boost::format("%d (%d)") % base % current));
	else
		printStatString(index, name, str(boost::format("%d") % base));
}
