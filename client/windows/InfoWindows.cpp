...
{
	CInfoWindow * window = create(text, player, components, DelComps);
	GH.pushInt(window);
}
...
void CRClickPopup::createAndPush(const CGObjectInstance * obj, const Point & p, EAlignment alignment /*= BOTTOMRIGHT*/)
{
	...
	else
	{
		if(adventureInt->curHero())
			createAndPush(obj->getHoverText(adventureInt->curHero()));
		else
			createAndPush(obj->getHoverText(LOCPLINT->playerID));
	}
}
