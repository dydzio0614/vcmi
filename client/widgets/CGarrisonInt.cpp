...
bool CGarrisonSlot::split()
{
	...
	GH.pushInt(new CSplitWindow(selection->creature, bind(&CGarrisonInt::splitStacks, owner, _1, _2),
...
CGarrisonSlot::CGarrisonSlot(CGarrisonInt * Owner, int x, int y, SlotID IID, EGarrisonType Upg, const CStackInstance * Creature):
	ID(IID),
	owner(Owner),
	myStack(Creature),
...
void CGarrisonInt::createSlots()
{
	...
	for(int i = 0; i < 2; i++)
	{
		...
		copy(garrisonSlots.begin(), garrisonSlots.end(), back_inserter(availableSlots));
	}
}
