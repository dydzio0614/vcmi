			...
			if(inBackpack) // Backpack destination.
			{
				...
				else
				{
					...
					else if(cur->isBig())
					{
						...
						LOCPLINT->showInfoDialog(str(boost::format(CGI->generaltexth->allTexts[153]) % cur->Name()));
					}
					else
					{
						setMeAsDest();
...
void CHeroArtPlace::setArtifact(const CArtifactInstance * art)
{
	...
	if(locked) // Locks should appear as empty.
		...
		hoverText = str(boost::format(CGI->generaltexth->heroscrn[1]) % ourArt->artType->Name());
}
	...
	allowedAssembling(true), highlightModeCallback(nullptr)
{
	if(createCommonPart)
	{
		commonInfo = std::make_shared<SCommonPart>();
