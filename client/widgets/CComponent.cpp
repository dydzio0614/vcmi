...
std::string CComponent::getSubtitleInternal()
{
	...
	{
	case primskill:
		return str(boost::format("%+d %s") % val % (subtype < 4 ? CGI->generaltexth->primarySkillNames[subtype] : CGI->generaltexth->allTexts[387]));
	case secskill:
...
{
	if(selected)
		return find(components.begin(), components.end(), selected) - components.begin();
	return -1;
}
