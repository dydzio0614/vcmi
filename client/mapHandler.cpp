...
bool CMapHandler::CMapPuzzleViewBlitter::canDrawObject(const CGObjectInstance * obj) const
{
	...
	if(find(unblittableObjects.begin(), unblittableObjects.end(), obj->ID) != unblittableObjects.end())
		return false;
...
void CMapHandler::getTerrainDescr(const int3 & pos, std::string & out, bool terName)
{
	...
	else if(terName)
	{
		...
		{
			out = str(boost::format("%s %s") % out % CGI->generaltexth->allTexts[330]); /// digging ok
		}
	}
}
...
IImage * CMapHandler::CMapCache::requestWorldViewCacheOrCreate(EMapCacheType type, const IImage * fullSurface)
{
