...
{
	listener(bind(&CSoundHandler::onVolumeChange, this, _1));
...
Mix_Chunk * CSoundHandler::GetSoundChunk(std::string & sound, bool cache)
{
	try
	{
		...
		if(cache)
			soundChunks.insert(std::pair<std::string, CachedChunk>(sound, make_pair(chunk, move(data.first))));
...
{
	listener(bind(&CMusicHandler::onVolumeChange, this, _1));
	// Map music IDs
	// Vectors for helper
...
void MusicEntry::load(std::string musicURI)
{
	...
	auto musicFile = MakeSDLRWops(CResourceHandler::get()->load(ResourceID(move(musicURI), EResType::MUSIC)));
