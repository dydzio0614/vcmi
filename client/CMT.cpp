...
#endif
{
	...
	if(argc > 1)
	{
		try
		{
			store(parse_command_line(argc, argv, opts, po_style::unix_style | po_style::case_insensitive), vm);
		}
		catch(std::exception & e)
		{
	...
	*console->cb = processCommand;
	...
	{
		if(!fileToStartFrom.empty() && exists(fileToStartFrom))
