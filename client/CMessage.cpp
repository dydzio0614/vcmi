...
std::pair<T, U> max(const std::pair<T, U> & x, const std::pair<T, U> & y)
{
	std::pair<T, U> ret;
...
std::vector<std::string> CMessage::breakText(std::string text, size_t maxLineWidth, EFonts font)
{
	...
	trim_right_if(text, boost::algorithm::is_any_of(std::string(" ")));
	...
	while(text.length())
	{
		...
		else
		{
			...
			trim_left_if(text, boost::algorithm::is_any_of(std::string(" ")));
		}
