...
CAttackAnimation::CAttackAnimation(CBattleInterface * _owner, const CStack * attacker, BattleHex _dest, const CStack * defender)
	: CBattleStackAnimation(_owner, attacker),
	  soundPlayed(false), dest(_dest),
	  shooting(false),
	  group(CCreatureAnim::SHOOT_FRONT), attackedStack(defender), attackingStack(attacker)
{
...
bool CMovementAnimation::shouldRotate()
{
	...
	if((begPosition.x < endPosition.x) && owner->creDir[stack->ID] == false)
	{
		return true;
	}
	return false;
