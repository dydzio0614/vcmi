...
void CBattleInterface::bSurrenderf()
{
	...
	if(cost >= 0)
	{
		...
		std::string surrenderMessage = str(boost::format(CGI->generaltexth->allTexts[32]) % enemyHeroName % cost); //%s states: "I will accept your surrender and grant you and your troops safe passage for the price of %d gold."
...
void CBattleInterface::bSpellf()
{
	...
	else if(spellCastProblem == ESpellCastProblem::MAGIC_IS_BLOCKED)
	{
		...
		if(blockingBonus->source == Bonus::ARTIFACT)
		{
			...
			LOCPLINT->showInfoDialog(str(boost::format(CGI->generaltexth->allTexts[683])
...
bool CBattleInterface::canStackMoveHere(const CStack * activeStack, BattleHex myNumber)
{
	...
	if(vstd::contains(acc, myNumber))
		return true;
	if(activeStack->doubleWide() && vstd::contains(acc, shiftedDest))
		return true;
	return false;
}
...
void CBattleInterface::handleHex(BattleHex myNumber, int eventType)
{
	...
	if(currentAction > INVALID)
	{
		...
		case AIMED_SPELL_CREATURE:
			...
			consoleMsg = str(boost::format(CGI->generaltexth->allTexts[27]) % sp->name % shere->getName()); //Cast %s on %s
			switch(sp->id)
			{
		...
		case ANY_LOCATION:
			sp = CGI->spellh->objects[creatureCasting ? creatureSpellToCast : spellToCast->additionalInfo]; //necessary if creature has random Genie spell at same time
			consoleMsg = str(boost::format(CGI->generaltexth->allTexts[26]) % sp->name); //Cast %s
			isCastingPossible = true;
			break;
		case RANDOM_GENIE_SPELL: //we assume that teleport / sacrifice will never be available as random spell
			sp = nullptr;
			consoleMsg = str(boost::format(CGI->generaltexth->allTexts[301]) % shere->getName()); //Cast a spell on %
	...
	else //no possible valid action, display message
	{
		...
		case FREE_LOCATION:
			cursorFrame = ECursor::COMBAT_BLOCKED;
			consoleMsg = str(boost::format(CGI->generaltexth->allTexts[181]) % sp->name); //No room to place %s here
			break;
		default:
			if(myNumber == -1)
	...
	case 12: //from bottom right
		{
			...
			if(vstd::contains(occupyableHexes, destHex))
				return destHex;
			if(activeStack->side == BattleSide::ATTACKER)
			{
...
CBattleInterface::SiegeHelper::~SiegeHelper()
{
	...
	for(int g = 0; g < ARRAY_COUNT(walls); ++g)
	{
		if(g != GATE || (gateState != EGateState::NONE && gateState != EGateState::CLOSED && gateState != EGateState::BLOCKED))
...
void CBattleInterface::SiegeHelper::printPartOfWall(SDL_Surface * to, int what)
{
	...
	if(town->town->faction->index == ETownType::TOWER
		&& (what == MOAT || what == BACKGROUND_MOAT))
...
BattleObjectsByHex CBattleInterface::sortObjectsByHex()
{
	...
	for(auto & stack : stacks)
	{
		...
		if((creAnims[stack->ID]->getType() == CCreatureAnim::DEAD || creAnims[stack->ID]->getType() == CCreatureAnim::HOLDING) && stack->isGhost());//ignore
